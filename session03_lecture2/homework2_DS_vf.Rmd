...
```
Most Likely Categories for Fraud:
Shopping_net: Online shopping transactions are the most likely to be fraudulent.
Grocery_pos: In-store grocery transactions also show a high likelihood of fraud.
Misc_net: Miscellaneous online transactions have a similar high likelihood of fraud.
Shopping_pos: In-store shopping transactions have a moderate likelihood of fraud.
Gas_transport: Transactions related to transportation and gas have a notable likelihood of fraud.
...
```
Days: Fraudulent transactions are more likely to occur on Thursdays and Fridays.
Hours: The peak hours for fraudulent transactions are late at night, between 11 PM and 2 AM.
Months: January and February are the months with the highest percentage of fraudulent transactions.
...
```
Yes, older customers are significantly more likely to be victims of credit card fraud. The histograms show a shift towards an older age group for fraudulent transactions, and the t-test confirms that the difference in mean ages between fraudulent and non-fraudulent transactions is statistically significant.

-   Is fraud related to distance? The distance between a card holder's home and the location of the transaction can be a feature that is related to fraud.
...
```
Based on the boxplot and violin plot, distance does not appear to be a strong distinguishing feature between legit and fraudulent transactions. The medians, interquartile ranges, and overall distributions are very similar for both categories, indicating that distance alone is unlikely to be a useful feature in explaining or predicting fraud.
...
co2_percap <- wb_data(country = "India", 
...
gdp_percap <- wb_data(country = "India", 
...
energy_data_country <- energy %>%
  filter(year >= 2000) %>%
  select(year, biofuel, coal, gas, hydro, nuclear, oil, other_renewable, solar, wind)

# Transform data to long format for plotting
energy_data_long <- energy_data_country %>%
  pivot_longer(cols = -year, names_to = "source", values_to = "value")

# Create the stacked area chart
ggplot(energy_data_long, aes(x = year, y = value, fill = source)) +
  geom_area(colour = "grey90", alpha = 0.5, position = "fill") +
  labs(title = paste("Electricity Generation in India Since 2000"),
       x = "Year", y = "Proportion of Total Electricity Generation",
       fill = "Energy Source") +
  theme_bw()
```
...
# Merge energy, CO2 emissions, and GDP per capita datasets
merged_data <- energy %>%
  inner_join(co2_percap, by = c("iso3c", "year")) %>%
  inner_join(gdp_percap, by = c("iso3c", "year"))

# Create the scatter plot for CO2 emissions per capita and GDP per capita
ggplot(merged_data, aes(x = GDPpercap, y = co2percap)) +
  geom_point(alpha = 0.6) +
  labs(title = "Relationship between CO2 Emissions per Capita and GDP per Capita",
...
# Convert electricity usage per capita from kWh/year to kWh/day
merged_data <- merged_data %>%
  mutate(electricity_per_capita_per_day = per_capita_electricity / 365)

# Create the scatter plot
ggplot(merged_data, aes(x = GDPpercap, y = electricity_per_capita_per_day)) +
  geom_point(alpha = 0.6) +
  labs(title = "Relationship between Electricity Usage per Capita/Day and GDP per Capita",
       x = "GDP per Capita (PPP, constant 2017 international $)",
       y = "Electricity Usage per Capita/Day (kWh)") +
  theme_bw()
```
...
A. I would use pivot_longer to convert the energy dataset from wide to long format. The cols argument specifies the columns to pivot (the different energy sources), names_to specifies the new column name for the energy source types, and values_to specifies the new column name for the values of electricity generated.
...
# Function to generate and arrange the three plots
generate_country_plots <- function(country_name) {
  # Download and clean the electricity data
  url <- "https://nyc3.digitaloceanspaces.com/owid-public/data/energy/owid-energy-data.csv"
  ...
  # Download and clean the CO2 emissions data
  co2_percap <- wb_data(country = country_name, 
                        indicator = "EN.ATM.CO2E.PC", 
                        start_date = 1990, 
                        end_date = 2022,
                        return_wide = FALSE) %>% 
  ...
  # Download and clean the GDP per capita data
  gdp_percap <- wb_data(country = country_name, 
                        indicator = "NY.GDP.PCAP.PP.KD", 
                        start_date = 1990, 
                        end_date = 2022,
                        return_wide = FALSE) %>% 
  ...
  # Create the stacked area chart
  p1 <- ggplot(energy_data_long, aes(x = year, y = value, fill = source)) +
    geom_area(colour = "grey90", alpha = 0.5, position = "fill") +
  ...
  # Create the scatter plot for CO2 emissions per capita and GDP per capita
  p2 <- ggplot(merged_data, aes(x = GDPpercap, y = co2percap)) +
    geom_point(alpha = 0.6) +
    geom_text_repel(aes(label = year), size = 2, max.overlaps = 20) +
  ...
  # Create the scatter plot for CO2 emissions per capita and electricity usage per capita/day
  p3 <- ggplot(merged_data, aes(x = electricity_per_capita_per_day, y = co2percap)) +
    geom_point(alpha = 0.6) +
    geom_text_repel(aes(label = year), size = 2, max.overlaps = 20) +
...
country_plots <- generate_country_plots("India")
ggsave("country_plots.png", plot = country_plots, width = 16, height = 9)
print(country_plots)
