...
# Join the donations and parties tables
party_joined_data <- donations %>%
  inner_join(parties, by = c("party_id" = "id"))

# Calculate the total amount donations made by an entity
most_donations <- party_joined_data %>% 
  group_by(entity) %>% 
  summarise(total_amount = sum(value, na.rm = TRUE), 
            perc_donation = value/total_amount) %>% 
...
```{r}
# Filter data to get relevant columns and rows
...
# Create the plot
ggplot(cfr_data, aes(x = age_group, y = cfr, fill = "#F09481")) +
